 ...
 */
export async function fetchResearchDomains(): Promise<
  ResearchDomain[]
> {

  const res = await fetch(
    `${API_BASE_URL}/research-domains`
  );


  const json =
    await handleResponse<ResearchDomainsResponse>(res);
...
export async function fetchResearchDomainById(
  ...
  const res = await fetch(
    `${API_BASE_URL}/research-domains/${id}`
  );


  const json =
    await handleResponse<ResearchDomainResponse>(res);
 ...
 */
export async function createResearchDomain(
  data: {
    title: string;
    slug: string;
    description: string;
 ...
 */
export async function deleteResearchDomain(
  id: string
): Promise<void> {
