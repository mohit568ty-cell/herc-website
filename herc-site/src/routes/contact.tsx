...
/** Client-side validation; server integration wires in later. */
  ...
  return (
    <section className="relative overflow-hidden pb-12 pt-36 sm:pt-44">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        >
          Reach out for research collaborations, consultancy enquiries, project
          discussions or media requests. Every enquiry is reviewed personally.
        </motion.p>
      </div>
    </section>
  );
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(body?.message ?? "Unable to send your enquiry.");
      }
        ...
        >
          ...
          <div className="mt-8 flex flex-wrap items-center justify-between gap-4">
            <p className="text-xs text-muted-foreground">
              ...
              <a href="/privacy" className="text-forest underline-offset-2 hover:underline">
                privacy policy
              </a>
              .
            </p>
          ...
          {status === "error" && submitError && (
            <p role="alert" className="mt-6 rounded-2xl border border-destructive/40 bg-destructive/10 px-5 py-4 text-sm text-destructive">
              {submitError}
            </p>
          )}
        </motion.form>
  ...
  return (
    <div>
      <Label htmlFor={id} className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
        {label}
        {required && <span className="text-emerald-glow"> *</span>}
      </Label>
...
function InfoCard({
  ...
  const inner = (
    <div className="flex items-start gap-4 rounded-2xl border border-border bg-card p-5 shadow-elegant transition-colors hover:border-emerald-glow/40">
      <div className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-secondary text-forest">
    ...
    <section className="py-8">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="overflow-hidden rounded-3xl border border-border bg-card shadow-elegant">
          <div
            className="relative flex h-64 items-center justify-center bg-secondary/60 sm:h-80"
          ...
          >
            ...
            <div className="relative text-center">
              <MapPin className="mx-auto h-6 w-6 text-forest" />
  ...
  return (
    <section className="py-16 sm:py-20">
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        ...
        />
        <div className="mt-12 divide-y divide-border rounded-2xl border border-border bg-card shadow-elegant">
          {faqs.map((f) => (
            <details key={f.q} className="group px-6 py-5 open:bg-secondary/40">
              <summary className="flex cursor-pointer items-center justify-between gap-4 text-sm font-medium text-foreground">
                <span className="flex items-center gap-3">
                  <MessageSquare className="h-4 w-4 text-emerald-glow" />
                  {f.q}
                </span>
